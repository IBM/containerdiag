...
# usage: ps -elf | grep debug-node | debugpodinfo.awk [-v "fssearch=value"]
{
  for (i = 0; i < NF-1; i++) {
    if ($i == "--persist-dir") {
      ...
      if (verbose) {
        print "debugpodinfo: " persistdir " : " result > "/dev/stderr";
        print "debugpodinfo: " $0;
      }
      
      if (system("test -d /host/" result "/" fssearch) == 0) {
        while ((cmd | getline result) > 0) {
          print result;
        }
      } else {
        if (verbose) {
          print "debugpodinfo: File not found: /host/" result "/" fssearch > "/dev/stderr";
        }
      }
